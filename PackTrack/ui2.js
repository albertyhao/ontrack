// if chrome storage is false

function geoCallback(e) {
  let lat = e["coords"]["latitude"];
  let lng = e["coords"]["longitude"];

  let url = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key=AIzaSyAGJTu_uVM6rDZcLIgoTj3nXmIXcVRw73s`
  Http.open("GET", url);
  Http.send();
}

chrome.storage.sync.get(['location'], (res) => {
  if (!res["location"]) {
    const state_codes = ["AL", "AK", "AZ", "AR", "CA", "CO", "CT", "DE", "FL", "GA", "HI", "ID", "IL", "IN", "IA", "KS", "KY", "LA", "ME", "MD", "MA", "MI", "MN", "MS", "MO", "MT", "NE", "NV", "NH", "NJ", "NM", "NY", "NC", "ND", "OH", "OK", "OR", "PA", "RI", "SC", "SD", "TN", "TX", "UT", "VT", "VA", "WA", "WV", "WI", "WY", "DC", "MH", "AE", "AA", "AE", "AE", "AE", "AP"]

    const Http = new XMLHttpRequest();

    const Geolocation = navigator.geolocation;
    Geolocation.getCurrentPosition(geoCallback)



    Http.onreadystatechange=(e) => {
      let base_json = JSON.parse(Http.responseText);
      let address = base_json["results"]["0"]["address_components"]

      var state = "";

      for (i in address) {
        if (state_codes.includes(address[i]["short_name"])) {
      ...
      // add this shit to the chrome storage
      chrome.storage.sync.set({location: state}, null)
    }
  }
})
